...
describe('Cleanup Archived Images Command', function (): void {
    beforeEach(function (): void {
        Storage::fake('public');
        Config::set('lawn.storage.archive.enabled', true);
    });

    it('skips cleanup when archiving is disabled', function (): void {
    ...
    it('handles no images to cleanup', function (): void {
        $this->artisan('lawn:cleanup-archived-images')
            ->expectsOutput('No images to cleanup.')
            ->assertExitCode(0);
    });

    it('cleans up archived images', function (): void {
    ...
    it('handles partial failures during cleanup', function (): void {
